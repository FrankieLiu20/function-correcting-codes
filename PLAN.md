...
| `#theorem 4#` | IV | over `F₂`, if `2 ≤ |Im f| ≤ k` then `r_f(k,t_d,t_f) ≥ 2t_f + t_d` | `binary_optimalRedundancyData_ge` | proved |
| `#theorem 5#` | IV | for an `[n,k,2t_d+1]` code `C`: `N(DRM) ≤ N(CDRM) + n − k`, with `C` in systematic form (which the paper's proof assumes WLOG; `ISSUES.md` §13(a)) | `N_drmData_le_N_cdrm_add` | proved |
| `#corollary 4#` | IV | `r_f(k,t_d,t_f) ≤ N(D_{C,f}) + n − k` (with `C` systematic, and the paper's standing `d_d ≤ d_f`) | `optimalRedundancyData_le_N_cdrm_add` | proved |
...
| `#theorem 6#` | IV | `N(D_{C,f}(t_f:u₁,…,u_M)) ≤ N(M, 2(t_f−t_d))` | `N_cdrm_le_Nconst` | proved |
...
| `#theorem 7#` | IV | for a systematic `[n,k,2t_d+1]` code `C`: the two-step scheme (`r_s = (n−k) + r'`, modelled by `twoStepCode`/`IsSecondStep`) satisfies `N(CDRM) + n−k ≤ r_s`, and `r_s = N(CFDM) + n−k` is achieved by any CFDM `D`-code | `two_step_redundancy_bounds` | proved |
...
The **Status** column is the authority on where the project is; the commit
messages say `phase 3.x` and match this table's `#` column (the theorem-2 work
was committed as 3.1 for its four bricks and 3.2 for the assembled identity, so
the labels 3.1/3.2 refer to that one row).

| # | Contents | Why this order | Status |
| --- | --- | --- | --- |
